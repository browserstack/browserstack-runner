var Log = require('./logger'),
  ...
  localBinary = __dirname + (windows ? '/BrowserStackLocal.exe' : '/BrowserStackLocal'),
  utils = require('./utils'),
...
var Tunnel = function Tunnel(key, port, uniqueIdentifier, callback, err) {
  ...
  fs.exists(localBinary, function(exists) {
    ...
    (windows ? "http://s3.amazonaws.com/browserStack/browserstack-local/BrowserStackLocal.exe" : ("http://s3.amazonaws.com/browserStack/browserstack-local/BrowserStackLocal-" + process.platform + "-" + process.arch)),
    function(response) {
      response.pipe(file);
