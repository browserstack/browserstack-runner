...
exports.Server = function Server(bsClient, workers, tracebacks) {
  ...
  handlers = {
    ...
    "_report": function reportHandler(uri, body, request, response) {
      ...
      if (query.tracebacks.length > 0) {
        ...
        status += query.tracebacks.length;
      }

      console.log("[%s] Completed in %d miliseconds. %d of %d passed, %d failed.",
