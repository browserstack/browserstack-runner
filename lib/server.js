...
exports.Server = function Server(bsClient, workers) {
  ...
  function prettyOutput(traceback, worker) {
    var details = traceback; //JSON.parse(traceback);
    var loc = details.testName;
    var output = chalk.red("FAILED: ") + loc + " : " + (details.message ? details.message : "");
    if (details.actual) {
